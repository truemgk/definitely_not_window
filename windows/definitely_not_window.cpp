...
namespace dn_window
{
    ...
    {
        has_custom_frame = (flags & DN_CUSTOM_FRAME);
        visible_on_startup = !(flags & DN_HIDE_ON_STARTUP);
        return 1;
    }
    ...
    void handle_dn_action(HWND window, WPARAM action, LPARAM actionData)
    {
        switch (action)
        {
        case DN_SETWINDOWPOS:
        {
    ...
    LRESULT CALLBACK main_window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam, UINT_PTR subclssID, DWORD_PTR refData)
    {
        ...
        case WM_NCCREATE:
        {
            ...
            SetProp(window, L"LaaviWindow", (HANDLE)(1));
            break;
        }
        case WM_NCHITTEST:
        {
        ...
        case WM_DN_ACTION:
        {
            handle_dn_action(window, wparam, lparam);
            break;
        }
        default:
            break;
...
} // laavi_window namespace

int dn_window_configure(unsigned int flags)
{
    return dn_window::configure(flags);
}